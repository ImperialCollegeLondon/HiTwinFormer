...
wandb.init(
    project="HiSiNet",
    name=f"{args.model_name}_lr{args.learning_rate}_bs{args.batch_size}_seed{args.seed}_lf{args.loss}",
    config=vars(args)
)
...
# Model setup
model = eval("models." + args.model_name)(mask=args.mask, image_size=args.image_size, triplet=is_triplet).to(cuda)
...
for epoch in range(1, args.epoch_training + 1):
    ...
    if loss_function=="contrastive":
        for batch_idx, (x1, x2, labels) in enumerate(train_loader):
            ...
            x1, x2, labels = x1.to(cuda), x2.to(cuda), labels.to(cuda)
