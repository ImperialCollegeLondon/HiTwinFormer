...
def summarize_distances(
    ...
    start_col: str = "Start",
    middle_col: str = "Middle",
    label_col: str = "labels_siam"
) -> pd.DataFrame:
    grouped = (
        df
        .groupby([chr_col, start_col, middle_col, label_col])['distances']
        .mean()
        .reset_index()
    )
    
    pivot = grouped.pivot_table(
        index=[chr_col, start_col, middle_col],
        columns=label_col,
        values='distances'
    )
    ...
    # Simple positive difference (between - within)
    pivot['distance_simple'] = (pivot['between'] - pivot['within']).clip(lower=0)
...
def process_checkpoint(args, ckpt_path):
    ...
    for chrom in all_chroms:
        ...
        df = pd.DataFrame({
            ...
            "Start": start_positions,   # actual tile start site
            "Middle": middle_positions, # middle bin (for plotting)
            "distances": distances,
